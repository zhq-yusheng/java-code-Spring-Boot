...
public class SocketServer {
	 ...
	 */
	private static CopyOnWriteArraySet<WebSocketVo> socketServers = new CopyOnWriteArraySet<>();
	 ...
	 */
	private final static String SYS_USERNAME = "lg";
	...
	public void open(Session session, @PathParam(value="userName")String userName){
			...
			socketServers.add(new WebSocketVo(userName,session));
            //这添加数据库
	...
	public void onMessage(String message){
		...
		WebSocketVo webSocketVo = socketServers.stream().filter(cli -> cli.getSession() == session)
				.collect(Collectors.toList()).get(0);
		sendMessage(webSocketVo.getUserName()+"<--"+message,SYS_USERNAME);

		logger.info("客户端:【{}】发送信息:{}",webSocketVo.getUserName(),message);
	}
	...
	@OnClose
	public void onClose(){
		socketServers.forEach(webSocketVo ->{
			if (webSocketVo.getSession().getId().equals(session.getId())) {
                //数据库删除用户
				logger.info("客户端:【{}】断开连接",webSocketVo.getUserName());
				socketServers.remove(webSocketVo);
    ...
    public void onError(Throwable error) {
		socketServers.forEach(webSocketVo ->{
			if (webSocketVo.getSession().getId().equals(session.getId())) {
				socketServers.remove(webSocketVo);
				logger.error("客户端:【{}】发生异常",webSocketVo.getUserName());
	...
	public synchronized static void sendMessage(String message,String userName) {
		...
		socketServers.forEach(webSocketVo ->{
			if (userName.equals(webSocketVo.getUserName())) {
				try {
					webSocketVo.getSession().getBasicRemote().sendText(message);

					logger.info("服务端推送给客户端 :【{}】",webSocketVo.getUserName(),message);
	...
	public synchronized static int getOnlineNum(){
		return socketServers.stream().filter(webSocketVo -> !webSocketVo.getUserName().equals(SYS_USERNAME))
	...
	public synchronized static List<String> getOnlineUsers(){
		...
		List<String> onlineUsers = socketServers.stream()
				.filter(webSocketVo -> !webSocketVo.getUserName().equals(SYS_USERNAME))
				.map(webSocketVo -> webSocketVo.getUserName())
